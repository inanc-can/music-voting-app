...
export default function PlayBox() {
  ...
  if (song === undefined) {
    <div
      ...
      onClick={fetchWinnerSong}
    >
      Fetch Winner Song
    </div>;
    return (
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center rounded-2xl 2xl:max-w-80 bg-gray-900 p-4">
        <h1 className="text-white font-semibold">No Song is Being Played</h1>
        <h1 className="text-white font-semibold">{winnerSong?.name}</h1>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center rounded-2xl 2xl:max-w-80 bg-gray-900 p-4">
        ...
        <div className="flex-col px-4">
          ...
          <h2 className="text-gray-400">Next Song: {winnerSong?.name}</h2>
        </div>
      </div>
    </div>
  );
